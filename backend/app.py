...
def get_destination(dest_id):
    ...
    if 0 <= dest_id < len(destinations):
        return jsonify(destinations[dest_id])
...
def update_profile():
    ...
    return jsonify({
        ...
        'user': {
            ...
            'preferences': users[user_index]['preferences']
        }
    })
...
def admin_update_destination(dest_id):
    ...
    if dest_id < 0 or dest_id >= len(destinations):
        return jsonify({'error': 'Destination not found'}), 404
    
    # Update destination fields
    for key in ['name', 'type', 'region', 'cost', 'weather', 'best_season', 'activities', 'safety_rating', 'user_rating', 'image']:
        if key in data:
            destinations[dest_id][key] = data[key]
    
    save_json(DESTINATIONS_FILE, destinations)
    
    return jsonify({'message': 'Destination updated', 'destination': destinations[dest_id]})
...
def admin_delete_destination(dest_id):
    ...
    deleted = destinations.pop(dest_id)
    save_json(DESTINATIONS_FILE, destinations)
